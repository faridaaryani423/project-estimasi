...
const EstimasiForm = () => {
  ...
  const importFromExcel = (e) => {
    ...
    reader.onload = (evt) => {
      try {
        ...
        const namaClient = cleanExcelVal(rows[5]?.[1]);    // ← NEW
        const perusahaan = cleanExcelVal(rows[6]?.[1]);
        const lokasi = cleanExcelVal(rows[7]?.[1]);       // ← NEW
        const kontakPerson = cleanExcelVal(rows[8]?.[1]); // ← NEW
  ...
  const saveManualBarangPermanent = async (index) => {
    ...
    try {
      ...
      const freshData = await barangAPI.getAll();
      setBarangList(freshData);
          ...
          <CardContent className="space-y-4">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
              ...
              <div className="space-y-2">
                ...
                <Input name="kontakPerson" value={formData.kontakPerson} onChange={handleInputChange} placeholder="Contoh: 08123456789" />
              </div>
            </div>
